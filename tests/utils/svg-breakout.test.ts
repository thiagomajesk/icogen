...
test("parseSvgBreakout extracts defs, labels paths, and preserves parent transforms", () => {
  ...
  assert.deepEqual(
    parsed.paths.map((path) => path.label),
    ["snake 1", "snake 2", "wing", "Circle 1", "box"],
  );
  assert.match(parsed.paths[2].markup, /transform="translate\(2 4\)"/);
  assert.match(parsed.paths[2].markup, /opacity="0.5"/);
});
...
test("buildForegroundStyledSvg applies per-path styles, gradients, and blink ordering", () => {
  const breakout = {
    viewBox: "0 0 64 64",
    defs: `<linearGradient id="existing"><stop offset="0%" stop-color="#000"/></linearGradient>`,
    inner: "",
    paths: [
      { id: "path-1", label: "one", markup: `<path fill="#000" d="M1 1L2 2Z" />` },
      { id: "path-2", label: "two", markup: `<path style="fill:#fff;stroke:#222" d="M2 2L3 3Z" />` },
      { id: "path-3", label: "three", markup: `<circle fill="#00ff00" cx="5" cy="5" r="2" />` },
      { id: "path-4", label: "four", markup: `<rect x="1" y="1" width="4" height="4" />` },
    ],
  };
  ...
  const svg = buildForegroundStyledSvg(
    ...
    { pathId: "path-2", token: 42 },
  );

  assert.match(svg, /id="aikon-path-hover-vignette"/);
  assert.match(svg, /id="existing"/);
  assert.match(svg, /id="fg-path-gradient-1"/);
  assert.match(svg, /id="fg-path-inner-shadow-1"/);
  assert.match(svg, /data-foreground-path-id="path-1"/);
  assert.match(svg, /fill="#123456"/);
  assert.match(svg, /data-foreground-path-id="path-2" data-blink-token="42"/);
  ...
  assert.match(svg, /fill:url\(#fg-path-gradient-1\)/);
  assert.match(svg, /stroke:#ff00ff/);
  assert.match(svg, /stroke-width:2/);
  assert.match(svg, /stroke-dasharray:8,4/);
  assert.match(svg, /data-foreground-path-id="path-3" filter="url\(#fg-path-inner-shadow-1\)"/);
  assert.match(svg, /fill="#00ff00"/);
  assert.match(svg, /data-foreground-path-id="path-4"/);
  assert.match(
    svg,
  ...
  const path1Index = svg.indexOf('data-foreground-path-id="path-1"');
  const path2Index = svg.indexOf('data-foreground-path-id="path-2"');
  const path4Index = svg.indexOf('data-foreground-path-id="path-4"');
  assert.equal(path2Index > path4Index && path4Index > path1Index, true);
});
