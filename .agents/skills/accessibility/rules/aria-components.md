...
function MenuPopup(props: any) {
  ...
  return (
    <ul
      {...menuProps}
      ref={ref}
      className="absolute top-full left-0 mt-1 min-w-[200px] bg-white border rounded shadow-lg py-1 z-50"
    >
...
function MenuItem({ item, state, onAction, onClose }: any) {
  ...
  return (
    <li
      {...menuItemProps}
      ref={ref}
      className={`px-4 py-2 cursor-pointer ${isFocused ? "bg-blue-50" : ""} ${isPressed ? "bg-blue-100" : ""}`}
    >
      {item.rendered}
    </li>
  );
