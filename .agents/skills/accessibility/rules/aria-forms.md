    ...
    <div className="relative inline-flex flex-col">
      ...
      {state.isOpen && (
        <ul
          {...listBoxProps}
          ref={listBoxRef}
          className="absolute top-full w-full border bg-white"
        >
          {[...state.collection].map((item) => (
            <li key={item.key} className="px-3 py-2 hover:bg-gray-100">
              {item.rendered}
            </li>
          ))}
        </ul>
...
function TextField(props) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input {...inputProps} ref={ref} className="border rounded px-3 py-2" />
      {props.description && (
